...
EVENTO_RESP="$(curl -s -X POST "$BASE/api/eventos" \
  ...
  -d "{
    ...
    \"descripcion\":\"Prueba end to end final\",
    \"lugarId\":\"$LUGAR_ID\",
...
section "6. EVENTOS: PUBLICAR SIN SECCIONES DEBE FALLAR 400"
...
PUBLICAR_SIN_SEC_CODE="$(curl -s -o /tmp/publicar_sin_sec.json -w "%{http_code}" -X PUT "$BASE/api/eventos/$EVENTO_SIN_SEC_ID/publicar" \
  -H "Authorization: Bearer $ADMIN_TOKEN")"
assert_code "publicar evento sin secciones debe dar 400" "$PUBLICAR_SIN_SEC_CODE" "400"
cat /tmp/publicar_sin_sec.json | print_json

section "7. INVENTARIO DIRECTO: RESERVAR, LIBERAR Y SOBREVENTA"

...
assert_error_contract() {
  ...
  if grep -q '"codigo_error"' "$file"; then
    green "OK  - $name usa codigo_error"
    OK=$((OK+1))
  else
    red "FAIL - $name no usa codigo_error"
    FAIL=$((FAIL+1))
  fi

  if grep -q '"codigoError"' "$file"; then
    red "FAIL - $name todavía usa codigoError"
    FAIL=$((FAIL+1))
  else
    green "OK  - $name no usa codigoError"
    OK=$((OK+1))
  fi
}
...
cat /tmp/no_token_reservas.json | print_json
assert_error_contract "gateway sin token reservas" /tmp/no_token_reservas.json

code="$(curl -s -o /tmp/no_token_pagos.json -w "%{http_code}" -X POST "$BASE/api/pagos" \
  -H "Content-Type: application/json" -d '{}')"
assert_code "POST /api/pagos sin token debe rechazar" "$code" "401"
cat /tmp/no_token_pagos.json | print_json
assert_error_contract "gateway sin token pagos" /tmp/no_token_pagos.json
...
assert_error_contract "ms-usuarios duplicado" /tmp/dup_email.json
...
assert_error_contract "ms-usuarios login incorrecto" /tmp/bad_login.json
...
section "5. EVENTOS: CREAR CON SECCIONES Y PUBLICAR SIN BODY"
...
EVENTO_RESP="$(curl -s -X POST "$BASE/api/eventos" \
  ...
  -d "{
    ...
    \"descripcion\":\"Prueba end to end contract\",
    \"lugarId\":\"$LUGAR_ID\",
...
[ -n "$EVENTO_ID" ] && [ "$EVENTO_ESTADO" = "BORRADOR" ] && green "OK  - evento creado BORRADOR con secciones en request" && OK=$((OK+1)) || { red "FAIL - evento no quedó BORRADOR"; FAIL=$((FAIL+1)); exit 1; }

PUBLICAR_CODE="$(curl -s -o /tmp/publicar_evento.json -w "%{http_code}" -X PUT "$BASE/api/eventos/$EVENTO_ID/publicar" \
  -H "Authorization: Bearer $ADMIN_TOKEN")"
assert_code "publicar evento SIN body" "$PUBLICAR_CODE" "200"
cat /tmp/publicar_evento.json | print_json

PUBLICAR_ESTADO="$(json_get "$(cat /tmp/publicar_evento.json)" "datos.estado")"
...
SECCIONES_COUNT="$(JSON_INPUT="$INV_RESP" python3 - <<'PY'
import json, os
try:
    print(len(json.loads(os.environ["JSON_INPUT"]).get("datos") or []))
except Exception:
    print(0)
PY
)"
[ "$SECCIONES_COUNT" -ge 2 ] && green "OK  - inventario inicializado con $SECCIONES_COUNT secciones desde Kafka" && OK=$((OK+1)) || { red "FAIL - inventario no inicializó las 2 secciones"; FAIL=$((FAIL+1)); exit 1; }
...
[ -n "$SECCION_ID" ] && green "OK  - sección principal=$SECCION_ID precio=$SECCION_PRECIO disp=$SECCION_DISP" && OK=$((OK+1)) || { red "FAIL - no se pudo leer sección principal"; FAIL=$((FAIL+1)); exit 1; }

section "6. CONTRATO DE ERRORES POR SERVICIO"
LUGAR2_RESP="$(curl -s -X POST "$BASE/api/lugares" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{
    \"nombre\":\"Venue Sin Secciones $RUN_ID\",
    \"direccion\":\"Zona 11\",
    \"ciudad\":\"Guatemala\",
    \"capacidadMaxima\":5000
  }")"
LUGAR2_ID="$(json_get "$LUGAR2_RESP" "datos.id")"

EVENTO_SIN_SEC_RESP="$(curl -s -X POST "$BASE/api/eventos" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{
    \"nombre\":\"Evento Sin Secciones $RUN_ID\",
    \"descripcion\":\"Debe fallar al publicar\",
    \"lugarId\":\"$LUGAR2_ID\",
    \"fechaEvento\":\"2026-12-31T20:00:00\",
    \"tipoEvento\":\"MIXTO\"
  }")"
EVENTO_SIN_SEC_ID="$(json_get "$EVENTO_SIN_SEC_RESP" "datos.id")"

PUBLICAR_SIN_SEC_CODE="$(curl -s -o /tmp/publicar_sin_sec.json -w "%{http_code}" -X PUT "$BASE/api/eventos/$EVENTO_SIN_SEC_ID/publicar" \
  -H "Authorization: Bearer $ADMIN_TOKEN")"
assert_code "ms-eventos publicar sin secciones debe dar 400" "$PUBLICAR_SIN_SEC_CODE" "400"
cat /tmp/publicar_sin_sec.json | print_json
assert_error_contract "ms-eventos publicar sin secciones" /tmp/publicar_sin_sec.json

RESERVA_NO_EXISTE_CODE="$(curl -s -o /tmp/reserva_no_existe.json -w "%{http_code}" "$BASE/api/reservas/00000000-0000-0000-0000-000000000000" \
  -H "Authorization: Bearer $CLIENT_TOKEN")"
assert_code "ms-reservas reserva no existe debe dar 404" "$RESERVA_NO_EXISTE_CODE" "404"
cat /tmp/reserva_no_existe.json | print_json
assert_error_contract "ms-reservas reserva no existe" /tmp/reserva_no_existe.json

PAGO_NO_EXISTE_CODE="$(curl -s -o /tmp/pago_no_existe.json -w "%{http_code}" "$BASE/api/pagos/00000000-0000-0000-0000-000000000000" \
  -H "Authorization: Bearer $CLIENT_TOKEN")"
assert_code "ms-pagos pago no existe debe dar 404" "$PAGO_NO_EXISTE_CODE" "404"
cat /tmp/pago_no_existe.json | print_json
assert_error_contract "ms-pagos pago no existe" /tmp/pago_no_existe.json
...
assert_code "ms-inventario sobreventa debe dar 409" "$SOBREVENTA_CODE" "409"
cat /tmp/inv_sobreventa.json | print_json
assert_error_contract "ms-inventario sobreventa" /tmp/inv_sobreventa.json

section "7. INVENTARIO DIRECTO: RESERVAR Y LIBERAR"
...
section "8. RESERVAS: CREAR, IDEMPOTENCIA Y CONSULTA"
...
try:
    print(1 if Decimal("$RESERVA_PRECIO") == Decimal("$SECCION_PRECIO") else 0)
except Exception:
    print(0)
PY
)"
assert_bool "precio de reserva coincide con precio de sección" "$PRICE_OK"
...
section "9. PAGOS: IDEMPOTENCIA, OUTBOX Y TRANSICIÓN DE RESERVA"
...
section "10. PAGOS 70/30: OBSERVAR COMPLETADO Y FALLIDO"
SEEN_COMPLETADO=0
SEEN_FALLIDO=0
...
for i in $(seq 1 12); do
  ...
  RES_LOOP="$(curl -s -X POST "$BASE/api/reservas" \
    -H "Authorization: Bearer $CLIENT_TOKEN" \
    -H "Idempotency-Key: idem-res-${RUN_ID}-loop-$i" \
    -H "Content-Type: application/json" \
    -d "{
      \"eventoId\":\"$EVENTO_ID\",
      \"items\":[
...
assert_error_contract "gateway cliente no puede crear evento" /tmp/client_create_event.json
...
section "13. LOGS RECIENTES PARA KAFKA"
...
docker logs soldout-ms-eventos --tail 100 2>/dev/null | grep -E "evento.creado|SECCIONES_REQUERIDAS|ERROR" || true
echo "--- ms-inventario evento.creado ---"
docker logs soldout-ms-inventario --tail 140 2>/dev/null | grep -E "evento.creado|INVENTARIO INICIALIZADO|ERROR" || true
echo "--- ms-pagos outbox ---"
docker logs soldout-ms-pagos --tail 140 2>/dev/null | grep -E "OUTBOX|PAGO PROCESADO|ERROR" || true
echo "--- ms-reservas pago confirmado/fallido ---"
docker logs soldout-ms-reservas --tail 140 2>/dev/null | grep -E "pago.confirmado|pago.fallido|RESERVA CONFIRMADA|RESERVA EXPIRADA|ERROR" || true
echo "--- ms-notificaciones ---"
docker logs soldout-ms-notificaciones --tail 140 2>/dev/null | grep -E "NOTIFICACION|pago.confirmado|pago.fallido|reserva.expirada|ERROR" || true
